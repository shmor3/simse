...
describe('Error States E2E', () => {
	...
	// -----------------------------------------------------------------
	it('invalid command shows "Unknown command" error', async () => {
	...
	// -----------------------------------------------------------------
	it('empty prompt submission keeps prompt active', async () => {
		term = await createSimseTerminal({ acpBackend: 'none' });
		await waitForReady(term);
	...
	// -----------------------------------------------------------------
	it('very long input (300+ chars) does not crash the app', async () => {
		term = await createSimseTerminal({ acpBackend: 'none' });
		await waitForReady(term);
