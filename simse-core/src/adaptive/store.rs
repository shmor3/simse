...
use crate::adaptive::cataloging::{MagnitudeCache, MetadataIndex, TopicIndex};
use crate::adaptive::distance::{compute_magnitude, DistanceMetric};
use crate::adaptive::deduplication;
use crate::adaptive::error::AdaptiveError;
use crate::adaptive::fusion;
use crate::adaptive::graph::{GraphConfig, GraphIndex};
use crate::adaptive::index::HnswConfig;
use crate::adaptive::inverted_index::InvertedIndex;
use crate::adaptive::learning::{LearningEngine, LearningOptions};
use crate::adaptive::persistence::{self, AccessStats};
use crate::adaptive::recommendation::{
	compute_recommendation_score, frequency_score, normalize_weights, recency_score,
	RecommendationScoreInput,
};
use crate::adaptive::text_cache::TextCache;
use crate::adaptive::text_search::{self, matches_all_metadata_filters};
use crate::adaptive::topic_catalog::TopicCatalog;
...
impl Store {
	...
	pub fn initialize(mut self, storage_path: Option<&str>) -> Result<Self, AdaptiveError> {
		...
		for i in 0..self.volumes.len() {
			for j in (i + 1)..self.volumes.len() {
				let sim = crate::adaptive::distance::cosine_similarity(
	...
	pub fn add(
		...
		for i in 0..new_idx {
			...
			let sim = crate::adaptive::distance::cosine_similarity_with_magnitude(
	...
	pub fn get_profile(&self) -> Option<crate::adaptive::types::LearnerProfile> {
	...
	pub fn graph_neighbors(
		&self,
		id: &str,
		edge_types: Option<&[crate::adaptive::graph::EdgeType]>,
		max_results: usize,
	) -> Vec<(&crate::adaptive::graph::Edge, Option<&Entry>)> {
	...
	pub fn graph_traverse(
		...
		edge_types: Option<&[crate::adaptive::graph::EdgeType]>,
		max_results: usize,
	) -> Vec<(crate::adaptive::graph::TraversalNode, Option<&Entry>)> {
...
fn combine_scores(
	...
	rank_by: &str,
	rank_weights: Option<&crate::adaptive::types::RankWeights>,
) -> Option<f64> {
	match rank_by {
		"vector" => vector_score,
		"text" => text_score,
