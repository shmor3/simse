...
use crate::engine::mcp::http_transport::{HttpTransport, HttpTransportConfig};
...
impl RetryConfig {
	fn to_shared(&self) -> crate::engine::acp::resilience::RetryConfig {
		crate::engine::acp::resilience::RetryConfig {
...
{
	crate::engine::acp::resilience::retry_generic(&config.to_shared(), is_transient, f).await
}
...
impl TransportKind {
	...
	async fn connect(&mut self) -> Result<crate::engine::mcp::protocol::McpInitializeResult, McpError> {
		match self {
...
mod tests {
	...
	fn on_logging_message_registers_handler() {
		...
		let msg = LoggingMessage {
			level: crate::engine::mcp::protocol::LoggingLevel::Info,
		...
		assert_eq!(guard[0].level, crate::engine::mcp::protocol::LoggingLevel::Info);
	}
