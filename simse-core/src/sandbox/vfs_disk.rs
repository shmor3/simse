...
use crate::sandbox::vfs_diff::{compute_diff, DiffOutput};
use crate::sandbox::error::SandboxError;
use crate::sandbox::vfs_glob::{expand_braces, match_parts};
use crate::sandbox::vfs_types::{DirEntry, HistoryEntry, ReadFileResult, SearchResult, StatResult};
use crate::sandbox::vfs_search::{search_text, SearchMode, SearchOptions};
...
impl DiskFs {
    ...
    pub fn resolve_path(&self, file_path: &str) -> Result<PathBuf, SandboxError> {
        ...
        let canonical = fs::canonicalize(&joined).map_err(|e| {
            SandboxError::VfsNotFound(format!("{}: {}", joined.display(), e))
        })?;
    ...
    pub fn resolve_parent_path(&self, file_path: &str) -> Result<PathBuf, SandboxError> {
        let local = strip_file_scheme(file_path)?;
        let local = local.strip_prefix('/').unwrap_or(local);
        ...
        let parent = joined.parent().ok_or_else(|| {
            SandboxError::VfsInvalidPath("Cannot determine parent directory".into())
        })?;

        let file_name = joined.file_name().ok_or_else(|| {
            SandboxError::VfsInvalidPath("Cannot determine file name".into())
        })?;

        let canonical_parent = fs::canonicalize(parent).map_err(|e| {
            SandboxError::VfsNotFound(format!("{}: {}", parent.display(), e))
        })?;
    ...
    fn check_sandbox(&self, canonical: &Path, original: &str) -> Result<PathBuf, SandboxError> {
        ...
        Err(SandboxError::VfsInvalidPath(format!(
    ...
    pub fn read_file(&self, path: &str) -> Result<ReadFileResult, SandboxError> {
        let resolved = self.resolve_path(path)?;

        if !resolved.is_file() {
            return Err(SandboxError::VfsNotAFile(path.to_string()));
        }
            ...
            if let Some(parent) = joined.parent() {
                ...
                let canonical_ancestor =
                    fs::canonicalize(check_path).map_err(|e| {
                        SandboxError::VfsNotFound(format!("{}: {}", check_path.display(), e))
                    })?;
    ...
    pub fn append_file(&self, path: &str, content: &str) -> Result<(), SandboxError> {
        let resolved = self.resolve_path(path)?;
    ...
    pub fn exists(&self, path: &str) -> Result<bool, SandboxError> {
        match self.resolve_path(path) {
            ...
            Err(SandboxError::VfsNotFound(_)) => Ok(false),
            Err(e) => Err(e),
        }
    }
    ...
    pub fn mkdir(&self, path: &str, recursive: bool) -> Result<(), SandboxError> {
        let local = strip_file_scheme(path)?;
        let local = local.strip_prefix('/').unwrap_or(local);
        let joined = self.root_directory.join(local);
    ...
    pub fn readdir(&self, path: &str, recursive: bool) -> Result<Vec<DirEntry>, SandboxError> {
        let resolved = self.resolve_path(path)?;

        if !resolved.is_dir() {
            return Err(SandboxError::VfsNotADirectory(path.to_string()));
        }
    ...
    /// Remove a directory.
    pub fn rmdir(&self, path: &str, recursive: bool) -> Result<(), SandboxError> {
        let resolved = self.resolve_path(path)?;
        ...
        if recursive {
            ...
            fs::remove_dir(&resolved).map_err(|e| {
                if e.kind() == std::io::ErrorKind::PermissionDenied {
                    return SandboxError::VfsPermissionDenied(format!("{}: {}", path, e));
                }
                ...
                {
                    SandboxError::VfsNotEmpty(path.to_string())
                } else {
    ...
    pub fn copy(
        ...
        if resolved_src.is_file() {
            if resolved_dest.exists() && !overwrite {
                return Err(SandboxError::VfsAlreadyExists(dest.to_string()));
            }
            ...
            return Err(SandboxError::VfsNotFound(src.to_string()));
        }
    ...
    pub fn glob(&self, patterns: &[String]) -> Result<Vec<String>, SandboxError> {
    ...
    pub fn tree(&self, path: &str) -> Result<String, SandboxError> {
        let resolved = self.resolve_path(path)?;
    ...
    pub fn search(
        ...
        let compiled_regex = if matches!(opts.mode, DiskSearchMode::Regex) {
            Some(Regex::new(query).map_err(|e| {
                SandboxError::VfsInvalidOperation(format!("Invalid regex: {}", e))
            })?)
        } else {
            None
        };
    ...
    pub fn history(&self, path: &str) -> Result<Vec<HistoryEntry>, SandboxError> {
        let _ = self.resolve_path(path)?; // validate path is in sandbox
        let history = self.history.as_ref().ok_or(SandboxError::VfsInvalidOperation(
    ...
    pub fn diff(
        ...
        compute_diff(&old_lines, &new_lines, context, u32::MAX).map_err(|e| {
            SandboxError::VfsInvalidOperation(format!("Diff failed: {}", e))
        })
    }
    ...
    pub fn checkout(&self, path: &str, version: usize) -> Result<(), SandboxError> {
        let resolved = self.resolve_path(path)?;
        let history = self.history.as_ref().ok_or(SandboxError::VfsInvalidOperation(
            "History not configured".into(),
        ))?;
...
impl DiskHistory {
    ...
    fn ensure_dirs(&self) -> Result<(), SandboxError> {
    ...
    pub fn store_content(&self, data: &[u8]) -> Result<String, SandboxError> {
        self.ensure_dirs()?;
    ...
    pub fn load_content(&self, hash: &str) -> Result<Vec<u8>, SandboxError> {
        if hash.len() < 3 {
            return Err(SandboxError::VfsInvalidOperation("Invalid hash".into()));
        }
        let object_path = self.objects_dir.join(&hash[..2]).join(&hash[2..]);
        fs::read(&object_path).map_err(|_| {
            SandboxError::VfsNotFound(format!("History object not found: {}", hash))
        })
    }
    ...
    fn read_manifest(&self, file_path: &str) -> Result<FileManifest, SandboxError> {
        ...
        serde_json::from_str(&data).map_err(|e| SandboxError::VfsJson(e.to_string()))
    }

    fn write_manifest(&self, manifest: &FileManifest) -> Result<(), SandboxError> {
        self.ensure_dirs()?;
    ...
    pub fn get_history(&self, file_path: &str) -> Result<Vec<VersionEntry>, SandboxError> {
    ...
    pub fn get_version_content(
        ...
        file_path: &str,
        version: usize,
    ) -> Result<(String, Vec<u8>), SandboxError> {
        let manifest = self.read_manifest(file_path)?;
        let entry = manifest.versions.iter().find(|v| v.version == version).ok_or_else(|| {
            SandboxError::VfsNotFound(format!("Version {} not found for {}", version, file_path))
        })?;
...
/// Map OS permission-denied errors to `SandboxError::VfsPermissionDenied`.
///
/// All other I/O errors pass through as `SandboxError::VfsIo`.
fn map_io_error(e: std::io::Error, path: &str) -> SandboxError {
    if e.kind() == std::io::ErrorKind::PermissionDenied {
        SandboxError::VfsPermissionDenied(format!("{}: {}", path, e))
    } else {
        SandboxError::VfsIo(e.to_string())
    }
}
...
fn strip_file_scheme(path: &str) -> Result<&str, SandboxError> {
    path.strip_prefix(FILE_SCHEME).ok_or_else(|| {
        SandboxError::VfsInvalidPath(format!("Expected {} prefix: {}", FILE_SCHEME, path))
    })
}
...
fn walk_dir(dir: &Path, files: &mut Vec<PathBuf>) -> Result<(), SandboxError> {
    if !dir.is_dir() {
        return Ok(());
    }
...
/// the sandbox.
fn copy_dir_recursive(src: &Path, dest: &Path, overwrite: bool) -> Result<(), SandboxError> {
    if !dest.exists() {
    ...
    for entry in fs::read_dir(src)? {
        ...
        if ft.is_dir() {
            ...
            if dest_path.exists() && !overwrite {
                return Err(SandboxError::VfsAlreadyExists(
...
fn build_tree(dir: &Path, prefix: &str, output: &mut String) -> Result<(), SandboxError> {
...
fn disk_usage(path: &Path) -> Result<u64, SandboxError> {
    if path.is_file() {
