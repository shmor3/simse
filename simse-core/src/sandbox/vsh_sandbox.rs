...
impl SandboxConfig {
	...
	pub fn validate_cwd(&self, path: &Path) -> Result<PathBuf, SandboxError> {
		...
		Err(SandboxError::VshSandboxViolation(format!(
	...
	pub fn check_command(&self, command: &str) -> Result<(), SandboxError> {
		for pattern in &self.blocked_patterns {
			if command.contains(pattern) {
				return Err(SandboxError::VshSandboxViolation(format!(
