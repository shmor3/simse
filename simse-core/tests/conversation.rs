...
fn test_tool_result() {
	let conv = Conversation::new(None);
	let conv = conv.add_tool_result("call_1", "search", "found 3 results");
	let msgs = conv.messages();
...
fn test_max_messages_trimming() {
	...
	let conv = conv.add_user("1");
	let conv = conv.add_assistant("2");
	let conv = conv.add_user("3");
...
fn test_compact() {
	let conv = Conversation::new(None);
	let conv = conv.add_user("first");
	let conv = conv.add_assistant("response");
	let conv = conv.compact("summary of conversation");
...
fn test_serialize_deserialize() {
	let conv = Conversation::new(None);
	let conv = conv.set_system_prompt("system".into());
	let conv = conv.add_user("hello");
	let conv = conv.add_assistant("world");
	let json = conv.to_json();
	let conv2 = Conversation::new(None);
	let conv2 = conv2.from_json(&json);
...
fn test_needs_compaction() {
	...
	let conv = conv.add_user(&"x".repeat(60));
...
fn test_estimated_chars() {
	let conv = Conversation::new(None);
	let conv = conv.set_system_prompt("abc".into());
	let conv = conv.add_user("defgh");
...
fn test_replace_messages() {
	...
	let conv = conv.replace_messages(&new_msgs);
...
fn test_load_messages() {
	...
	let conv = conv.load_messages(msgs);
...
fn test_estimated_tokens_default() {
	let conv = Conversation::new(None);
	let conv = conv.add_user("abcdefgh"); // 8 chars => ceil(8/4) = 2 tokens
...
fn test_context_usage_percent_with_window() {
	...
	let conv = conv.add_user(&"a".repeat(40));
...
fn test_context_usage_percent_capped_at_100() {
	...
	let conv = conv.add_user(&"a".repeat(400));
...
fn test_serialize_format() {
	let conv = Conversation::new(None);
	let conv = conv.set_system_prompt("sys".into());
	let conv = conv.add_user("hello");
	let conv = conv.add_assistant("world");
...
fn test_serialize_tool_result_format() {
	let conv = Conversation::new(None);
	let conv = conv.add_tool_result("id1", "search", "found it");
	let s = conv.serialize();
...
fn test_to_messages_prepends_system() {
	let conv = Conversation::new(None);
	let conv = conv.set_system_prompt("be helpful".into());
...
fn test_trim_preserves_system_messages() {
	...
	let conv = conv.set_system_prompt("system prompt".into());
	let conv = conv.add_user("1");
	let conv = conv.add_assistant("2");
	let conv = conv.add_user("3");
...
fn test_compact_creates_user_summary() {
	let conv = Conversation::new(None);
	let conv = conv.add_user("a");
	let conv = conv.add_assistant("b");
	let conv = conv.compact("the summary");
	let msgs = conv.messages();
...
fn test_to_json_roundtrip_with_tool_result() {
	let conv = Conversation::new(None);
	let conv = conv.add_tool_result("tc_1", "my_tool", "result content");
	let json = conv.to_json();
	let conv2 = Conversation::new(None);
	let conv2 = conv2.from_json(&json);
...
fn test_needs_compaction_default_threshold() {
	...
	let conv = conv.add_user(&"x".repeat(50_000));
	assert!(!conv.needs_compaction());
	let conv = conv.add_user(&"y".repeat(60_000));
	assert!(conv.needs_compaction());
}
