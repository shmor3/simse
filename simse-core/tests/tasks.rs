...
#[test]
fn test_create_task() {
	let list = TaskList::new(None);
	let (list, task) = list.create(TaskCreateInput {
...
fn test_auto_increment_ids() {
	let list = TaskList::new(None);
	let (list, t1) = list.create(TaskCreateInput {
		subject: "A".into(),
		description: "".into(),
		active_form: None,
		owner: None,
	...
	let (_list, t2) = list.create(TaskCreateInput {
		subject: "B".into(),
		description: "".into(),
		active_form: None,
		owner: None,
...
fn test_get_task() {
	let list = TaskList::new(None);
	let (list, _) = list.create(TaskCreateInput {
		subject: "Task".into(),
...
fn test_delete_nonexistent_returns_false() {
	let list = TaskList::new(None);
	let (_list, deleted) = list.delete("999");
	assert!(!deleted);
}
...
fn test_delete_removes_task() {
	...
	let (list, deleted) = list.delete("1");
	assert!(deleted);
...
fn test_clear() {
	...
	assert_eq!(list.task_count(), 0);
	// IDs should reset
	let (_list, t) = list.create(TaskCreateInput {
		subject: "C".into(),
		description: "".into(),
		active_form: None,
		owner: None,
...
#[test]
fn test_task_limit() {
	let list = TaskList::new(Some(TaskListOptions { max_tasks: Some(2) }));
	let (list, _) = list.create(TaskCreateInput {
		subject: "A".into(),
		description: "".into(),
		active_form: None,
...
fn test_create_always_succeeds_under_limit() {
	let list = TaskList::new(Some(TaskListOptions { max_tasks: Some(5) }));
	let (_list, task) = list.create(TaskCreateInput {
		subject: "A".into(),
		description: "".into(),
		active_form: None,
		owner: None,
