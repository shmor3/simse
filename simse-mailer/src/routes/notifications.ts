...
notifications.get('/:userId', async (c) => {
	...
	const rows = await c.env.DB
		.prepare('SELECT id, type, title, body, read, link, created_at FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT 100')
		.bind(paramUserId)
		.all<{ id: string; type: string; title: string; body: string; read: number; link: string | null; created_at: string }>();
...
notifications.post('/', async (c) => {
	...
	const body = await c.req.json<{ userId: string; type: string; title: string; body: string; link?: string }>();
