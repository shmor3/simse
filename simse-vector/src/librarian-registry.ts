 ...
 * In the main simse package this is typically an ACPConnection;
 * simse-vector only requires a `close()` method so it can tear down
 * connections during `dispose()`.
 */
export interface DisposableConnection {
...
export interface ManagedLibrarian {
	...
	readonly connection?: DisposableConnection;
}
