...
impl Default for LocalNetBackend {
    fn default() -> Self {
        Self::new()
    }
}
...
impl LocalNetBackend {
    /// Create a new `LocalNetBackend` with a default `reqwest::Client`.
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
    ...
    /// Create a `LocalNetBackend` with a custom `reqwest::Client`.
...
#[async_trait]
impl NetBackend for LocalNetBackend {
    async fn http_request(
        ...
        let reqwest_method: reqwest::Method = method
            .parse()
            .map_err(|_| VnetError::InvalidParams(format!("invalid HTTP method: {method}")))?;
        ...
        let response = builder.send().await.map_err(|e| {
            if e.is_timeout() {
                VnetError::Timeout(format!("HTTP request timed out after {timeout_ms}ms"))
            } else {
                VnetError::ConnectionFailed(format!("HTTP request failed: {e}"))
            }
        })?;
        ...
        let body_bytes = response.bytes().await.map_err(|e| {
            VnetError::ConnectionFailed(format!("failed to read response body: {e}"))
        })?;
    ...
    async fn ws_connect(
        ...
        Err(VnetError::ConnectionFailed(
            "WebSocket via LocalNetBackend not yet implemented".to_string(),
        ))
    }
    ...
    async fn tcp_connect(&self, _host: &str, _port: u16) -> Result<String, VnetError> {
        Err(VnetError::ConnectionFailed(
            "TCP via LocalNetBackend not yet implemented".to_string(),
        ))
    }
    ...
    async fn udp_send(
        ...
        Err(VnetError::ConnectionFailed(
            "UDP via LocalNetBackend not yet implemented".to_string(),
        ))
    }
    ...
    async fn resolve(&self, hostname: &str) -> Result<Vec<String>, VnetError> {
        ...
        let addrs = lookup_host(&addr_str)
            .await
            .map_err(|e| VnetError::DnsResolutionFailed(format!("{hostname}: {e}")))?;
...
mod tests {
    ...
    fn default_creates_backend() {
        let backend = LocalNetBackend::default();
    ...
    fn with_client_creates_backend() {
        let client = Client::builder()
            ...
            .build()
            .unwrap();
        let backend = LocalNetBackend::with_client(client);
        drop(backend);
    }
    ...
    async fn ws_connect_returns_not_implemented() {
        ...
        let err = backend
            ...
            .await
            .unwrap_err();
        assert!(matches!(err, VnetError::ConnectionFailed(_)));
    ...
    async fn resolve_invalid_host() {
        ...
        let err = backend
            ...
            .await
            .unwrap_err();
        assert!(matches!(err, VnetError::DnsResolutionFailed(_)));
    }
    ...
    async fn http_request_invalid_method() {
        ...
        let err = backend
            ...
            .await
            .unwrap_err();
        assert!(matches!(err, VnetError::InvalidParams(_)));
    ...
    async fn http_request_connection_refused() {
        ...
        let err = backend
            ...
            .unwrap_err();
        assert!(
            matches!(err, VnetError::ConnectionFailed(_) | VnetError::Timeout(_)),
