...
/**
 * Convenience factory that creates a Librarian from a TextGenerationProvider.
 *
 * In the original simse package this accepted an ACPClient and internally
 * wrapped it.  In simse-vector the ACP dependency is removed — callers in
 * the main simse package should create a TextGenerationProvider from the
 * ACPClient themselves (via `createACPGenerator`) before calling this.
 */
export function createDefaultLibrarian(
