// ---------------------------------------------------------------------------
// MCP Engine Client — JSON-RPC 2.0 over NDJSON stdio transport to Rust subprocess
// ---------------------------------------------------------------------------
...
export interface McpEngineClientOptions {
	readonly enginePath: string;
	readonly timeoutMs?: number;
	readonly logger?: Logger;
}

export interface McpEngineClient {
...
export function createMcpEngineClient(
	options: McpEngineClientOptions,
): McpEngineClient {
	const timeoutMs = options.timeoutMs ?? 60_000;
	const logger = options.logger ?? createNoopLogger();
	const log = logger.child('mcp-engine-client');
	...
	const processLine = (line: string): void => {
		...
		if (isResponse(msg)) {
			...
			if (resp.error) {
				req.reject(
					createMCPError(
						`MCP engine error ${resp.error.code}: ${resp.error.message}`,
						{
							code: 'MCP_ENGINE_ERROR',
	...
	const sendRequest = <T>(method: string, params?: unknown): Promise<T> => {
		if (disposed || !child?.stdin?.writable) {
			return Promise.reject(
				createMCPConnectionError(
					'mcp-engine',
					'MCP engine client is not connected',
				),
			);
		}
		...
		return new Promise<T>((resolve, reject) => {
			const timer = setTimeout(() => {
				...
				reject(
					createMCPError(
						`MCP engine request "${method}" timed out after ${timeoutMs}ms`,
						{
							code: 'MCP_ENGINE_TIMEOUT',
	...
	const dispose = async (): Promise<void> => {
		...
		rejectAll(
			createMCPConnectionError('mcp-engine', 'MCP engine client disposed'),
		);
	...
	child.stderr?.on('data', (data: Buffer) => {
		...
		if (text) {
			log.debug('mcp-engine stderr', { text });
		}
	});

	child.on('error', (err) => {
		log.error('mcp-engine process error', toError(err));
		rejectAll(
			createMCPConnectionError(
				'mcp-engine',
				`MCP engine process error: ${toError(err).message}`,
				{ cause: err },
			),
		);
	});
	...
	child.on('exit', (code) => {
		if (!disposed) {
			log.warn('mcp-engine exited unexpectedly', { exitCode: code });
			rejectAll(
				createMCPConnectionError(
					'mcp-engine',
					`MCP engine exited with code ${code}`,
				),
			);
		}
	});

	log.info('MCP engine process spawned');
